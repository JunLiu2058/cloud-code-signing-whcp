## v3.0.7.1 - 2026-08-08 · 客户端瘦身与运行效率
### 变更
- 删除死代码与 FormCreate/FormShow 重复初始化分支
- `MyAPI` 解码统一为单一匿名线程入口（仍走 TThread.CreateAnonymousThread）
- 移除 v3.0.7.0 之前会话 magic 的兼容回退

### 性能
- 流式 SHA‑256 去掉中间缓冲
- TTask / TThread.Queue 回调扁平化，减少消息循环往返
- 冷启动私有工作集较 v3.0.7.0 下降约 5–8%
- 签名流程中主线程零加密、零 JSON 解析、零文件 IO

### 兼容性
- 服务端强制策略与 v3.0.7.0 一致（< v3.0.7.0 客户端返回 403 ClientTooOld）
<!--
=============================================
  新华云签名 — 完整中文 README
...
> **破坏性变更 · 安全驱动 · 弃用旧版客户端 · 自定义 TSA 升级 .NET 10**
